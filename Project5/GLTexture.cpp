...
GLTexture::GLTexture() : texturename(nullptr) {}
...
void GLTexture::Load(char* name)
{
	...
	strcpy_s(texturename, len, name);

	// ????? ?????? ???????
	for (size_t i = 0; i < len; i++)
		texturename[i] = static_cast<char>(tolower(texturename[i]));

	// ????? ?????? ????????
	char* quote = strchr(texturename, '\"');
	if (quote) *quote = '\0';
	...
	if (strstr(texturename, ".bmp")) {
		...
		MessageBoxA(NULL, "Unsupported texture format!", "Error", MB_OK);
	}
}
