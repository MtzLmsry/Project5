...
void Model_3DS::Load(char* name)
{
	...
	// ??? ????? ?????
	strncpy_s(filename, name, _TRUNCATE);

	// ????? ?????? "
	char* firstQuote = strchr(filename, '"');
	if (firstQuote)
	{
		...
		if (secondQuote)
		{
			...
			memmove(filename, firstQuote + 1, len);
			filename[len] = '\0';
		}
	}
	...
	{
		char* lastSlash1 = strrchr(filename, '/');
		char* lastSlash2 = strrchr(filename, '\\');
		char* temp = (lastSlash1 > lastSlash2) ? lastSlash1 : lastSlash2;

		size_t pathLen = temp - filename + 1;
	...
	// ---------- ??? ????? ????? ----------
	errno_t err = fopen_s(&bin3ds, filename, "rb");
	if (err != 0 || !bin3ds)
		return;
	...
	{
		if (!Materials[j].textured)
		{
			Materials[j].tex.BuildColorTexture(
				Materials[j].color.r,
				Materials[j].color.g,
				Materials[j].color.b
			);
...
void Model_3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
	...
	// ????? ?????? ?????? ???? ???
	char fullname[260] = { 0 };
	sprintf_s(fullname, sizeof(fullname), "%s%s", path, n.c_str());
